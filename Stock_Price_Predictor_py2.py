...
# If selecting using training date
...
print "The Top Features Selected are", top_features
...
ensemble_model_nn = models.ENSEMBLE_model_NN(train_compare,y_train)
...
ensemble_nn_preds = ensemble_model_nn.predict(test_compare)
ensemble_nn_r2_score = np.round(ensemble_model_nn.score(test_compare,y_test),5)
ensemble_nn_mae_score = np.round(mean_absolute_error(y_test,ensemble_nn_preds),5)
...
print "nn ensemble r-squared score =" , ensemble_nn_r2_score
...
print "nn ensemble MAE score =" , ensemble_nn_mae_score
...
future_ensemble_nn_preds = ensemble_model_nn.predict(future_compare)

preds = [future_ensemble_preds, Average_future, future_ensemble_svr_preds, future_ensemble_nn_preds]
...
txt = txt + "\n Lasso model 1 (using 3 variables) had a R-squared score = " + str(lasso_score)
txt = txt + "\n Lasso model 2 (using 20 variables) had a R-squared score = " + str(lasso2_score)
txt = txt + "\n Lasso model 3 (using PCA) had a R-squared score = " + str(lasso3_score)
txt = txt + "\n SGD model 1 (using 20 variables) had a R-squared  score = " + str(sgd_score)
txt = txt + "\n SGD model 2 (using PCA) had a R-squared score = " + str(sgd2_score)
txt = txt + "\n Linear Ensemble model had a R-squared score = {} and a MAE = {}".format(ensemble_r2_score, ensemble_mae_score)
txt = txt + "\n SVR Ensemble model had a R-squared score = {} and a MAE = {}".format(ensemble_svr_r2_score, ensemble_svr_mae_score)
#txt = txt + "\n KNR Ensemble model score = " + str(ensemble_knr_r2_score)
txt = txt + "\n NN Ensemble model had a R-squared score = {} and a MAE = {}".format(ensemble_nn_r2_score, ensemble_nn_mae_score)
txt = txt + "\n Average model had a R-squared score = {} and a MAE = {}".format(average_r2_score, average_mae_score)
txt = txt + "\n"
...
if ensemble_nn_r2_score >0:
	test_plt.plot(test_dates,ensemble_nn_preds,label='nn ensemble')
...
if ensemble_nn_r2_score >0:
	future_plt.plot(range(1,days +1),future_ensemble_nn_preds,label='nn ensemble')	
if average_r2_score>0:
	future_plt.plot(range(1,days +1),Average_future, label = 'average ensemble')
future_plt.legend(bbox_to_anchor =(1.05,1.05), loc = 2)
future_plt.text(days+6,(top-bottom)/2 + bottom,txt)
